...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = this.ascendingByRetweet0(new EmptyTrending)
  
  def ascendingByRetweet0(accu: Trending): Trending = {
    val m: Tweet = this.findMin
    val naccu: Trending = accu + m
    val nthis: TweetSet = this.remove(m)
    if (nthis.isEmpty) naccu else nthis.ascendingByRetweet0(naccu)
  }
