...
class FunSetSuite extends FunSuite {
	...
	test("forall and exists") {
	  new Evens {
	    assert(forall(se, x => (x % 2 == 0)))
	    assert(!exists(se, x => (x % 2 != 0)))
	  }
	}
	...
	test("double x") {
	  new Evens {
	    ...
	    printSet(s)
	    printSet(so)
	    printSet(map(so, double))
	    printSet(se)
	    printSet(map(se, double))
	    println(double(3))
	    printSet(map(s, double))
	    val s2: Set = map(s, double)
	    printSet(s2)
	    assert(forall(s2, even))
	  }
	}
}
